...
#include "MyOTAFirmwareUpdate_STM32F4.h"
...
SPIClass spiToUse(1); // int represents SPI port number
...
//			if (!_flash.initialize()) {
		...
		if (_firmwareUpdateOngoing) {
			...
			if (firmwareResponse->block != (uint16_t)(_firmwareBlock - 1)) {
	        ...
	        while(_flash.busy());
    	    _flash.WE();
        	_flash.writebyte( ((_firmwareBlock - 1) * FIRMWARE_BLOCK_SIZE) + FIRMWARE_START_OFFSET,
			                   firmwareResponse->data, FIRMWARE_BLOCK_SIZE);
